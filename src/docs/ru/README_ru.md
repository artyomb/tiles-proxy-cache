...
| `/api/stats` | GET | Сохранённая статистика по источникам без запуска пересчёта | JSON данные |
| `/api/stats/jobs` | POST | Поставить все источники на внеплановое обновление | JSON состояния |
| `/api/stats/sources/:source/refresh` | POST | Поставить один источник на обновление | JSON состояния |
